...
export interface ${capitalize(moduleName)}Model extends Model<T${capitalize(moduleName)}> {
...
const result = await ${capitalize(moduleName)}Services.create${capitalize(moduleName)}IntoDB(${moduleName}Data);
...
const { id } = req.params;
const result = await ${capitalize(moduleName)}Services.getSingle${capitalize(moduleName)}FromDB(id);
...
const result = await ${capitalize(moduleName)}Services.getAll${capitalize(moduleName)}sFromDB(req.query);
...
const result = await ${capitalize(moduleName)}Services.update${capitalize(moduleName)}IntoDB(id, ${moduleName});
...
const { id } = req.params;
const result = await ${capitalize(moduleName)}Services.delete${capitalize(moduleName)}FromDB(id);
...
if (!result) {
throw new AppError(httpStatus.BAD_REQUEST, 'Failed to create ${capitalize(moduleName)}');
}

return result;
};

const getAll${capitalize(moduleName)}sFromDB = async (query: Record<string, unknown>) => {
...
const update${capitalize(moduleName)}IntoDB = async (id: string, payload: any) => {
...
if (!deletedService) {
throw new AppError(httpStatus.BAD_REQUEST, 'Failed to delete ${capitalize(moduleName)}');
}
...
import { T${capitalize(moduleName)}, ${capitalize(moduleName)}Model } from './${moduleName}.interface';

const ${moduleName}Schema = new Schema<T${capitalize(moduleName)}, ${capitalize(moduleName)}Model>({
...
export const ${capitalize(moduleName)} = model<T${capitalize(moduleName)}, ${capitalize(moduleName)}Model>('${capitalize(moduleName)}', ${moduleName}Schema);
`,
},
{
